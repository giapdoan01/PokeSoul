...
public class KibluSkill : MonoBehaviour, IPokemonSkillLaunch
{
    [Header("Debug")]
    ...
    [SerializeField] private GameObject kibluImpact;
    ...
    {
        if (kibluImpact == null || SkillObjectPolling.Instance == null)
        {
            return;
        }

        SkillObjectPolling.Instance.RegisterPrefab(ImpactPoolKey, kibluImpact, impactPreloadAmount);
    ...
    private double ResolveDamageByLevel(PokemonData pokemonData, int level)
    {
        ...
        Debug.LogWarning("[KibluSkill] Missing 'damage' stat in PokemonData level. Damage = 0.");
        return 0;
    }
    ...
    private void SpawnExtraProjectiles(PokemonData pokemonData, int level, Transform targetEnemy, string attack)
    {
        ...
        for (int i = 1; i < projectileCount; i++)
        {
            ...
            KibluSkill extraSkill = extraProjectile.GetComponent<KibluSkill>();
            if (extraSkill == null)
            {
    ...
    private void SpawnImpactVfx()
    {
        if (kibluImpact == null)
        {
            LogDebug("kibluImpact is null, skip impact VFX.");
            return;
        }

        Vector3 spawnPosition = (target != null ? target.position : transform.position) + Vector3.up * impactHeightOffset;
        ...
        GameObject fallback = Instantiate(kibluImpact, spawnPosition, Quaternion.identity);
        LogDebug($"Impact VFX fallback instantiate: {fallback.name}");
        SetupFollower(fallback);
        if (impactLifeTime > 0f)
        {
    ...
    private void SetupFollower(GameObject impactGo)
    {
        if (target == null || impactGo == null) return;

        KibluImpactFollower follower = impactGo.GetComponent<KibluImpactFollower>() ?? impactGo.AddComponent<KibluImpactFollower>();

        follower.SetTarget(target, impactHeightOffset);
    }
    ...
    private void LogDebug(string message)
    {
        ...
        Debug.Log($"[KibluSkill:{name}] {message}", this);
    }
}
