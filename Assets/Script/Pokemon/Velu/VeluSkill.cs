...
public class VeluSkill : MonoBehaviour, IPokemonSkillLaunch
{
    [Header("Debug")]
    ...
    [SerializeField] private GameObject veluImpact;
    ...
    private const string ImpactPoolKey = "VeluImpact";
    ...
    {
        if (veluImpact == null || SkillObjectPolling.Instance == null) return;
        SkillObjectPolling.Instance.RegisterPrefab(ImpactPoolKey, veluImpact, impactPreloadAmount);
    }
    ...
    private double ResolveDamageByLevel(PokemonData pokemonData, int level)
    {
        if (pokemonData != null && pokemonData.TryGetStatValueByLevel(level, "damage", out double damage))
            return damage;
        Debug.LogWarning("[VeluSkill] Missing 'damage' stat. Damage = 0.");
        return 0;
    }
    ...
    private void SpawnExtraProjectiles(PokemonData pokemonData, int level, Transform targetEnemy, string attack)
    {
        ...
        for (int i = 1; i < projectileCount; i++)
        {
            ...
            VeluSkill extraSkill = extra.GetComponent<VeluSkill>();
    ...
    private void SpawnImpactVfx(Collider hitCollider, Vector3? overridePosition)
    {
        ...
        GameObject fallback = Instantiate(veluImpact, pos, Quaternion.identity);
    ...
    private void LogDebug(string message)
    {
        ...
        Debug.Log($"[VeluSkill:{name}] {message}", this);
    }
}
