...
public class KiakumaSkill : MonoBehaviour, IPokemonSkillLaunch
{
    [Header("Debug")]
    ...
    [SerializeField] private GameObject kiakumaImpact;
    ...
    private const string ImpactPoolKey = "KiakumaImpact";
    ...
    {
        if (kiakumaImpact == null || SkillObjectPolling.Instance == null) return;
        SkillObjectPolling.Instance.RegisterPrefab(ImpactPoolKey, kiakumaImpact, impactPreloadAmount);
    }
    ...
    private double ResolveDamageByLevel(PokemonData pokemonData, int level)
    {
        if (pokemonData != null && pokemonData.TryGetStatValueByLevel(level, "damage", out double damage))
            return damage;
        Debug.LogWarning("[KiakumaSkill] Missing 'damage' stat. Damage = 0.");
        return 0;
    }
    ...
    private void SpawnExtraProjectiles(PokemonData pokemonData, int level, Transform targetEnemy, string attack)
    {
        ...
        for (int i = 1; i < projectileCount; i++)
        {
            ...
            KiakumaSkill extraSkill = extra.GetComponent<KiakumaSkill>();
    ...
    private void SpawnImpactVfx(Collider hitCollider, Vector3? overridePosition)
    {
        ...
        GameObject fallback = Instantiate(kiakumaImpact, pos, Quaternion.identity);
    ...
    private void LogDebug(string message)
    {
        ...
        Debug.Log($"[KiakumaSkill:{name}] {message}", this);
    }
}
