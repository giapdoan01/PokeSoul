...
public class NibiSkill : MonoBehaviour, IPokemonSkillLaunch
{
    [Header("Debug")]
    ...
    [SerializeField] private GameObject nibiImpact;
    ...
    {
        if (nibiImpact == null || SkillObjectPolling.Instance == null)
        {
            return;
        }

        SkillObjectPolling.Instance.RegisterPrefab(ImpactPoolKey, nibiImpact, impactPreloadAmount);
    ...
    private double ResolveDamageByLevel(PokemonData pokemonData, int level)
    {
        ...
        Debug.LogWarning("[NibiSkill] Missing 'damage' stat in PokemonData level. Damage = 0.");
        return 0;
    }
    ...
    private void SpawnExtraProjectiles(PokemonData pokemonData, int level, Transform targetEnemy, string attack)
    {
        ...
        for (int i = 1; i < projectileCount; i++)
        {
            ...
            NibiSkill extraSkill = extraProjectile.GetComponent<NibiSkill>();
            if (extraSkill == null)
            {
    ...
    private void SpawnImpactVfx(Collider hitCollider, Vector3? overridePosition)
    {
        if (nibiImpact == null)
        {
            LogDebug("nibiImpact is null, skip impact VFX.");
            return;
        }
        ...
        GameObject fallback = Instantiate(nibiImpact, spawnPosition, Quaternion.identity);
    ...
    private void LogDebug(string message)
    {
        ...
        Debug.Log($"[NibiSkill:{name}] {message}", this);
    }
}
